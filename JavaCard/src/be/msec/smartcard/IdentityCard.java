...
public class IdentityCard extends Applet {
    ...
    private static final short SIZE_OF_CERT = (short)(ISSUER_LEN + SUBJECT_LEN + 2*DATE_LEN + EXPONENT_LEN + MODULUS_LEN + SIGN_LEN);
    ...
    private byte[] arrayOfOne = new byte[(short)1];
    private byte[] userUniqueKey = new byte[SIZE_OF_UNIQUE_KEY];
    private byte[] emptyResponse = new byte[(short)0];
    ...
    private byte[] dataFornym = new byte[(short)(SIZE_OF_UNIQUE_KEY + SUBJECT_LEN)];
    private byte[] subjectBuffer = new byte[SUBJECT_LEN];
    private byte[] hashedArray = new byte[(short)32];
    ...
    private byte[] concatChallengeAuth = new byte[(short)(SIZE_OF_CHALLENGE + SIZE_OF_AUTH)];
    private byte[] certificateWithoutSign = new byte [(short)(SIZE_OF_CERT - SIGN_LEN)];
    private byte[] paddedEncryptedData = new byte[(short)64];
    private byte[] encryptedChallenge = new byte[(short)32];
    private byte[] dataToEncrypt = new byte[(short)(SIZE_OF_CHALLENGE + SUBJECT_LEN)]; 
    private byte[] lenAndEncryptedPaddedChallenge = new byte[(short)(SIZE_OF_INT + SIZE_OF_PADDED_CHALLENGE)];
    private byte[] lenAndpaddedChallenge = new byte[(short)(SIZE_OF_INT + SIZE_OF_PADDED_CHALLENGE)];
    
    private byte[] paddedDataToEncrypt_1 = new byte[(short)32];
    private byte[] randomGeneratedData = new byte[(short)128];

    private byte[] response = new byte[(short)(4 + 4 + paddedEncryptedData.length +  encryptedChallenge.length)];
    private byte[] certificateAndSignature = new byte[(short)(SIGN_LEN + SIZE_OF_CERT)];
    private short  sizeToAdd = (short) (16 - (certificateAndSignature.length%16));
    private byte[] paddedDataToEncrypt = new byte[(short)(certificateAndSignature.length + sizeToAdd)];
    

    private byte[] encryptedCertificateAndSignature = new byte[(short)(certificateAndSignature.length + sizeToAdd)];
    private byte[] keybytes = new byte[(short)16];
    ...
    byte[] modulus = new byte[(short)(MODULUS_LEN+1)];
    ...
    private byte[] bigStorage = new byte[(short)1024];
    private byte[] intInBytes = new byte[]{0x00,0x00,0x00,0x00};
    private byte[] fourBytes = new byte[(short)4];
    
    private short authStep = 0;
    
    private byte[] nym = new byte[(short)32];
    private byte[] name = new byte[(short)32];
    private byte[] address = new byte[(short)48];
    private byte[] country = new byte[(short)2];
    private byte[] birthDate = new byte[(short)6];
    private byte donor;
    private byte age;
    private byte gender;
...
// max size byte array to release attributes
    private byte[] responseToAttributesReleasing = new byte[(short)(32 + 32 + 48 + 2 + 6 + 1 + 1 + 1 + 16)];
    private byte[] paddedResponseToAttributesReleasing = new byte[(short)(32 + 32 + 48 + 2 + 6 + 1 + 1 + 1 + 16 + 5)];//Multiple of 16 for encryption
    private byte[] encryptedPaddedResponseToAttributesReleasing = new byte[(short)(32 + 32 + 48 + 2 + 6 + 1 + 1 + 1 + 16 + 5)];//Multiple of 16 for encryption result
    ...
    private byte[] currentQuery = new byte[(short)9];
    ...
    private IdentityCard() {
        ...
        Util.arrayCopy(unpaddedName, SHORTZERO, name, SHORTZERO, (short) getMinimum((short)name.length, (short)unpaddedName.length));
        byte[] unpaddedAddress = {50,51,32,82,111,97,100,108,97,110,101,32,84,101,120,97,115};//"23 Roadlane Texas"
        Util.arrayCopy(unpaddedAddress, SHORTZERO, address, SHORTZERO, (short) getMinimum((short)address.length, (short)unpaddedAddress.length));
        
        byte[] unpaddedCountry = {66,69};
        Util.arrayCopy(unpaddedCountry, SHORTZERO, country, SHORTZERO, (short) getMinimum((short)country.length,(short) unpaddedCountry.length));
        ...
        byte[] timeBytes = new byte[(short)6];
        Util.arrayCopy(yearBytes, SHORTZERO, timeBytes, SHORTZERO,(short) 4);
        timeBytes[(short)4] = 4;
        timeBytes[(short)5] = 23;
    ...
    private static short getMinimum(short a, short b){
    	if (a>b){
    		return b;
    	}
    	return a;
    ...
    /**
    * Subtract two unsigned     egers represented in array A and B. The sum stored in
    * array C
   ...
   private static short getUnsignedByte(byte[] A, byte AOff, byte count) {
    ...
    private void verify_time_signature(APDU apdu) {
        ...
        short len = (short)(bigStorage[(short)0] << 24 | (bigStorage[(short)1] & 0xFF) << 16 | (bigStorage[(short)2] & 0xFF) << 8 | (bigStorage[(short)3] & 0xFF));
        //TODO
        ...
        arrayOfOne[(short)0] = (byte) response;
        apdu.setOutgoing();
    ...
    private byte[] intToByte(short value){
        intInBytes[(short)0] = 0;
        intInBytes[(short)1] = 0;
        intInBytes[(short)2] = (byte)(value >>> 8);
        intInBytes[(short)3] = (byte)value;
        return intInBytes;
    }
...
//          byte[] encryptedChallenge = aesenc.doFinal(dataToEncrypt);
            Util.arrayCopy(intToByte((short)paddedEncryptedData.length), (short) 0, response, (short) 0, (short) 4);
            Util.arrayCopy(intToByte((short)encryptedChallenge.length), (short) 0, response, (short) 4, (short) 4);
...
//          
    ...
    // step 4.5
        ...
        // step 4.6
            ...
            for (short i = 0; i < queryLen; i++) {
                byte requestedData = bigStorage[i];
                byte isRequestedDataAvailable = 0x00;
                for (short j = 0; j < accessibleFields.length; j++) {
                    byte b = accessibleFields[j];
                    if (b == requestedData) {
    ...
    private void end_auth(APDU apdu) {
        ...
        short size = (short) bytesToInt(fourBytes, SHORTZERO);
...
//      }
        ...
        short responseShort = (short) (lenAndEncryptedPaddedChallenge[(short)0] << 8 | (lenAndEncryptedPaddedChallenge[(short)1] & 0xFF));
        short previousChallenge = (short)  (challenge[(short)0] << 8 | (challenge[(short)1] & 0xFF));
        if (responseShort != (short)(previousChallenge+1)) {
    ...
    private static short bytesToInt(byte[] bytes, short offset){
        return (short) ((short)(bytes[(short)(offset+2)] & 0xFF) << 8 | (bytes[(short)(offset+3)] & 0xFF));
    }
